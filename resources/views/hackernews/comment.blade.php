<div class="comment card mt-3">
  <div class="card-body px-2 px-md-3 shadow {{ in_array($item->id, $collapsedComments) ? 'opacity-5' : '' }}">
    <h6 class="card-subtitle bg-light text-muted d-flex justify-content-between align-items-center">
      <span class="align-middle {{ $op === $item->by ? 'text-success' : (in_array($item->by, ['dang', 'sctb']) ? 'text-danger' : '') }}"
    ...
    <div id="collapse-{{ $item->id }}" class="mt-2 collapse {{ in_array($item->id, $collapsedComments) ? '' : 'show' }}">
